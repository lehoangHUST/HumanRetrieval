...
from yolov5.utils_yolov5.general import apply_classifier, check_img_size, check_imshow, check_requirements, \
        ...
        if type(pred_bbox_clothes) == torch.Tensor and type(pred_bbox_human) == torch.Tensor:
            ...
            for i in range(pred_bbox_human.shape[0]):
                ...
                for j in range(pred_bbox_clothes.shape[0]):
                    # Calculate area.
                    area_j = (pred_bbox_clothes[j][2] - pred_bbox_clothes[j][0]) * (
                                pred_bbox_clothes[j][3] - pred_bbox_clothes[j][1])
            ...
            if len(list_det) != 0:
                ...
                outputs = deepsort.update(xywhs.cpu(), confs.cpu(), clss.cpu(),
                                          img_numpy)  # np array (detections, [xyxy, track_id, class_id])
